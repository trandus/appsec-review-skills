...
- Map concrete `Finding` and `Candidate Finding` results after they are identified.
...
The local dataset contains all ASVS 5.0.0 requirements, with `L1`, `L2`, and `L3` applicability represented in each requirement's `levels` field. Keep using it only after a concrete `Finding` or `Candidate Finding` exists, in line with `sec-appsec-review`.
